 ...
 * the byte count the server reports, `revision` and `sha256` come from the API
 * rather than from anybody's memory, the licence is Apache 2.0, and the file
...
export interface ModelChoice {
  ...
  file: string;
  /**
   * The exact commit the weights are fetched from.
   *
   * Downloads used to go through `resolve/main/`, which is a branch and moves.
   * A repo owner replacing a quantisation in place would have changed what
   * Casebook installed with no commit here saying so — and would have silently
   * corrupted every resumed download, since the resume appends to a partial of
   * the file that no longer exists upstream.
   */
  revision: string;
  /** The file's SHA-256, from Hugging Face's API. Checked before it is installed. */
  sha256: string;
